...
import ViewTaskModal from "../../component/modals/ViewTaskModal";
...
const TaskTable = ({ onTaskAdded }) => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const tasksFromResponse = response.tasks.map((task) => ({
        id: task._id,
        name: task.name,
        assignedBy: task.assignerName,
            ...
            {currentTasks.map((task) => (
              <tr key={task.id} className="border-t text-gray-700">
                ...
                <td className="p-3 italic">
                  ...
                  >
                    {task.approval === "approved" ? task.approval : task.status}
                  </span>
                </td>
                <td className="p-3">
                  <ButtonSmallPurple
