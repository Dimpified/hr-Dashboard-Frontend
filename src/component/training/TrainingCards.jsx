...
const TrainingCards = ({ filterType }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
      {trainings.map((training, index) => (
        <div
          key={index}
          className="bg-white shadow-lg rounded-lg p-6 hover:scale-105 transition-all duration-300"
        >
          {/* Valid Till */}
          <div className="flex justify-between items-center mb-3">
            ...
            <button className="text-sec11 hover:text-primary11">
              <TbShare />
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 my-4">
            <span className="border text-primary11 px-3 py-1 rounded-lg text-xs font-medium">
              {training.rate}
            </span>
            <span className="border text-primary11 px-3 py-1 rounded-lg text-xs font-medium">
              {training.type}
